...
def check_swap(source_tokens, target_tokens):
    if source_tokens == [x for x in reversed(target_tokens)]:   # 使用率？
        return "$MERGE_SWAP"
    else:
        return None
...
def check_verb(source_token, target_token):
    ...
    if encoding:
        ...
        return f"$TRANSFORM_VERB-{target_token}"
    else:
        return None
...
def align_sequences(source_sent, target_sent):
    ...
    source_tokens = nltk.word_tokenize(source_sent)
    target_tokens = nltk.word_tokenize(target_sent)
